...
### Global Defines
TS2MAEV_VERSION = '0.2.0'
TS2MAEV_CONFIG = 'TS2MAEV.json'
...
def main():
    ...
    parser.add_argument(
        '-v','--version', action='version', version=TS2MAEV_VERSION)
    parser.add_argument(
        '--init', action='store_true',
        help='Create template client conf TS2MAEV.json only if there is no existing one')
    parser.add_argument(
        '-c','--conf', default=TS2MAEV_CONFIG,
        help='Configuration file. Default:TS2MAEV.json')
    parser.add_argument(
    ...
    if not os.path.exists(config_file):
        print_err(u"TS2MAEV.json file doesn't exist: {0}\n"
